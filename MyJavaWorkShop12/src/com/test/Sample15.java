...
class MyElement implements Comparable { //MyElement라는 타입을 생성 
	private int i;
	...
	static void BubbleSortArrayList(ArrayList al) {
		...
		while(!sorted) {
			...
			if(m3.compareTo(m4) < 0	) { 
				temp = m3;
				al.set(inx, m4); // 값 바꿔치기
				al.set(inx+1, temp); // 값 바꿔치기
				}
			}
		}
		for(Object m : al) {
...
//		System.out.println(m1.compareTo(m2)); // 크면 1
//		System.out.println(m2.compareTo(m1)); // 작으면 -1
//		System.out.println(m2.compareTo(m2)); // 같으면 0
